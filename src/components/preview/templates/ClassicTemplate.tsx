...
function PersonalHeader({ data }: { data: PersonalData }) {
  ...
  return (
    <header className="border-b border-black/15 pb-5">
      ...
      {data.title ? (
        <div className="mt-1 text-[15px] text-black/60 italic font-display">{data.title}</div>
      ) : null}
      <div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-1 text-[12px] text-black/70 font-mono">
      ...
      {links.length ? (
        <div className="mt-1.5 flex flex-wrap items-center gap-x-3 gap-y-1 text-[12px] text-black/60 font-mono">
          {links.map((l) => (
            <span key={l}>{l}</span>
          ))}
        </div>
    ...
    case 'summary': {
      ...
      return (
        <Section title="Summary">
          <p className="text-[13px] leading-relaxed text-black/80">{d.text}</p>
        </Section>
      )
    }
    case 'experience': {
            ...
            {d.items.map((it) => (
              <div key={it.id}>
                <div className="flex items-baseline justify-between gap-3">
                  <div>
                    <div className="text-[13.5px] font-semibold text-black tracking-tight">
                      {it.role || 'Role'}
                      {it.company ? (
                        <span className="font-normal text-black/70"> · {it.company}</span>
                      ) : null}
                    </div>
                    {it.location ? (
                      <div className="text-[12px] text-black/55">{it.location}</div>
                    ) : null}
                  </div>
                  <div className="text-[11.5px] font-mono uppercase tracking-wider text-black/55 whitespace-nowrap">
                    {it.start || '—'} → {it.current ? 'present' : it.end || '—'}
                  </div>
                </div>
                {it.bullets.length ? (
                  <ul className="mt-1.5 ml-4 list-[square] text-[12.5px] text-black/80 leading-relaxed marker:text-black/40">
                    {it.bullets.map((b) => (
              ...
              <div key={it.id} className="flex items-baseline justify-between gap-3">
                <div>
                  ...
                  <div className="text-[12px] text-black/65">
                    ...
                    {it.notes ? <span className="text-black/45"> — {it.notes}</span> : null}
                  </div>
                </div>
                <div className="text-[11.5px] font-mono uppercase tracking-wider text-black/55 whitespace-nowrap">
                  {it.start || '—'} → {it.end || '—'}
      ...
      return (
        <Section title="Skills">
          <div className="flex flex-col gap-1.5">
            {d.groups.map((g) => (
              <div key={g.id} className="text-[12.5px] text-black/85 leading-relaxed">
                <span className="font-semibold text-black">{g.label}: </span>
                <span className="text-black/75">{g.skills.join(' · ')}</span>
              </div>
            ))}
          </div>
        </Section>
            ...
            {d.items.map((it) => (
              <div key={it.id}>
                <div className="flex items-baseline justify-between gap-3">
                  ...
                  {it.url ? (
                    <div className="text-[11.5px] font-mono text-black/55">{it.url}</div>
                  ) : null}
                </div>
                {it.description ? (
                  <p className="text-[12.5px] text-black/80 leading-relaxed">{it.description}</p>
                ) : null}
                {it.tech.length ? (
                  <div className="mt-1 text-[11.5px] font-mono uppercase tracking-wide text-black/55">
                    {it.tech.join(' · ')}
                  </div>
                ) : null}
              </div>
            ...
            {d.items.map((it) => (
              <div key={it.id}>
                <div className="flex items-baseline justify-between gap-3">
                  ...
                  {it.subtitle ? (
                    <div className="text-[11.5px] font-mono uppercase tracking-wider text-black/55">
                      {it.subtitle}
                    </div>
                  ) : null}
                </div>
                {it.body ? (
                  <p className="text-[12.5px] text-black/80 leading-relaxed">{it.body}</p>
                ) : null}
              </div>
            ))}
          </div>
